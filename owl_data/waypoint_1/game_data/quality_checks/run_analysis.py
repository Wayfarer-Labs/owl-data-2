...
def check_darkness(metadata_json: dict, mp4_bytes: bytes) -> dict:
    ...
    """
    if metadata_json.get('game.exe') in ('obs.exe',):
...
def run_checks(tasks: list[str], out_path_jsonl: str, checks: str = 'menus,dark_screen,deduplicated') -> None:
...
def main() -> int:
    ...
    p.add_argument("--task-list", type=str, required=False, default=os.path.join(QUALITY_CHECKS_JSONL_TAR, "quality_checks.jsonl"), help="Text file with one local path per line")
    p.add_argument("--out-path-jsonl", type=str, required=False, default="/mnt/data/analysis.jsonl", help="Output prefix (local dir or s3://bucket/prefix)")
    ...
    p.add_argument("--include-checks", type=str, required=False, default="menus,dark_screen,deduplicated", help="One of the above quality checks of menus, dark_screen, deduplicated")
    ...
    checks = args.include_checks.split(',')
    
    run_checks(local_tasks, args.out_path_jsonl, *checks)
