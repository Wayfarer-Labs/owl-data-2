...
def hevc_to_chunks(hevc_path: Path) -> Generator[torch.Tensor, None, None]:
    """
    ...
    print(f"Processing {original_width}x{original_height} -> crop to {crop_width}x{crop_height} -> resize to 640x360")
    ...
    while True:
        ...
        # Apply transforms
        frame_processed = transform(frame_tensor)
...
def process_dongle_dir(dongle_dir: Path, out_dir: Path) -> None:
    ...
    for subdir in tqdm.tqdm(
        ...
        for i, hevc_chunk in tqdm.tqdm(
            enumerate(hevc_to_chunks(subdir / 'video.hevc')),
            ...
            torch.save(hevc_chunk, normalized_dir / f'{i:08d}.pt')
