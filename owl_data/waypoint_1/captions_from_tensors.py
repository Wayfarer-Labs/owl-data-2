...
from ..nn.captioner import VLMCaptioner
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(0, len(path_pairs)), desc=f"Captions (node {args.node_rank})", unit="video"):
        ...
        if len(fps_list) > args.batch_size:
            captions = []
            for i in range(0, len(fps_list), args.batch_size):
                batch_video_chunks = video_chunks[i:i+args.batch_size]
                batch_fps_list = fps_list[i:i+args.batch_size]
                batch_captions = captioner(batch_video_chunks, batch_fps_list)
                captions.extend(batch_captions)
        else:
            captions = captioner(video_chunks, fps_list) # list of list of dicts

        for j in range(len(captions)):
            with open(os.path.join(output_path, "splits", f"{j:08d}_captions.jsonl"), "w") as f:
                for caption in captions[j]:
                    json.dump(caption, f)
                    f.write("\n")
